@extends('layout.master')
@section('title','Templates')
@section('class-navbar','navbar-dark bg-dark')
@section('bg-body', 'bg-dark')

<style>
    
    .white-text{
        color: #f2f2f7;
    }
    #wb:hover{
        color: rgb(255, 59, 48);
    }
    #ma:hover{
        color: rgb(52, 199, 89);
    }
    #da:hover{
        color: rgb(50, 173, 230);
    }
    #h:hover{
        color: rgb(255, 149, 0);
    }

</style>
@section('content')

<div class="container bg-dark white-text" style="padding-top:5%; padding-bottom:5%">
    <div class="row g-3" style="text-align: center">
        <div class="col-md-12" style="padding-top:5%; padding-bottom:10%">
            <div class="row g-3">
                <div class="col-md-12">
                    <h3 data-aos="fade-up" data-aos-delay="200">Our Template are Compatible at this Platform</h3>
                </div>
                <div class="col-md-2"></div>
                <div class="col-md-8">
                    <div class="row g-3" data-aos="fade-up" data-aos-delay="200">
                        <div class="col-md-3">
                            <a href="#web-browser" id="wb" style="color:rgb(255, 69, 58)">
                                <svg xmlns="http://www.w3.org/2000/svg" width="90px" height="90px" fill="currentColor" class="bi bi-globe" viewBox="0 0 16 16">
                                <path d="M0 8a8 8 0 1 1 16 0A8 8 0 0 1 0 8zm7.5-6.923c-.67.204-1.335.82-1.887 1.855A7.97 7.97 0 0 0 5.145 4H7.5V1.077zM4.09 4a9.267 9.267 0 0 1 .64-1.539 6.7 6.7 0 0 1 .597-.933A7.025 7.025 0 0 0 2.255 4H4.09zm-.582 3.5c.03-.877.138-1.718.312-2.5H1.674a6.958 6.958 0 0 0-.656 2.5h2.49zM4.847 5a12.5 12.5 0 0 0-.338 2.5H7.5V5H4.847zM8.5 5v2.5h2.99a12.495 12.495 0 0 0-.337-2.5H8.5zM4.51 8.5a12.5 12.5 0 0 0 .337 2.5H7.5V8.5H4.51zm3.99 0V11h2.653c.187-.765.306-1.608.338-2.5H8.5zM5.145 12c.138.386.295.744.468 1.068.552 1.035 1.218 1.65 1.887 1.855V12H5.145zm.182 2.472a6.696 6.696 0 0 1-.597-.933A9.268 9.268 0 0 1 4.09 12H2.255a7.024 7.024 0 0 0 3.072 2.472zM3.82 11a13.652 13.652 0 0 1-.312-2.5h-2.49c.062.89.291 1.733.656 2.5H3.82zm6.853 3.472A7.024 7.024 0 0 0 13.745 12H11.91a9.27 9.27 0 0 1-.64 1.539 6.688 6.688 0 0 1-.597.933zM8.5 12v2.923c.67-.204 1.335-.82 1.887-1.855.173-.324.33-.682.468-1.068H8.5zm3.68-1h2.146c.365-.767.594-1.61.656-2.5h-2.49a13.65 13.65 0 0 1-.312 2.5zm2.802-3.5a6.959 6.959 0 0 0-.656-2.5H12.18c.174.782.282 1.623.312 2.5h2.49zM11.27 2.461c.247.464.462.98.64 1.539h1.835a7.024 7.024 0 0 0-3.072-2.472c.218.284.418.598.597.933zM10.855 4a7.966 7.966 0 0 0-.468-1.068C9.835 1.897 9.17 1.282 8.5 1.077V4h2.355z"/>
                              </svg>
                            <h5>Web Browser</h5>
                            </a>
                            
                        </div>
                        <div class="col-md-3">
                            <a href="#mobile-app" id="ma" style="color:rgb(49, 209, 88)">
                            <svg xmlns="http://www.w3.org/2000/svg" width="90px" height="90px" fill="currentColor" class="bi bi-phone" viewBox="0 0 16 16">
                                <path d="M11 1a1 1 0 0 1 1 1v12a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1h6zM5 0a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h6a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2H5z"/>
                                <path d="M8 14a1 1 0 1 0 0-2 1 1 0 0 0 0 2z"/>
                              </svg>
                            <h5>Mobile App</h5>
                            </a>
                            
                        </div>
                        <div class="col-md-3">
                            <a href="#desktop-app" id="da" style="color:rgb(10, 132, 255)">
                            <svg xmlns="http://www.w3.org/2000/svg" width="90px" height="90px" fill="currentColor" class="bi bi-laptop" viewBox="0 0 16 16">
                                <path d="M13.5 3a.5.5 0 0 1 .5.5V11H2V3.5a.5.5 0 0 1 .5-.5h11zm-11-1A1.5 1.5 0 0 0 1 3.5V12h14V3.5A1.5 1.5 0 0 0 13.5 2h-11zM0 12.5h16a1.5 1.5 0 0 1-1.5 1.5h-13A1.5 1.5 0 0 1 0 12.5z"/>
                              </svg>
                            <h5>Desktop App</h5>
                            </a>
                            
                        </div>
                        <div class="col-md-3">
                            <a href="#hybrid" id="h" style="color:rgb(255, 214, 10)">
                            <svg xmlns="http://www.w3.org/2000/svg" width="90px" height="90px" fill="currentColor" class="bi bi-arrow-repeat" viewBox="0 0 16 16">
                                <path d="M11.534 7h3.932a.25.25 0 0 1 .192.41l-1.966 2.36a.25.25 0 0 1-.384 0l-1.966-2.36a.25.25 0 0 1 .192-.41zm-11 2h3.932a.25.25 0 0 0 .192-.41L2.692 6.23a.25.25 0 0 0-.384 0L.342 8.59A.25.25 0 0 0 .534 9z"/>
                                <path fill-rule="evenodd" d="M8 3c-1.552 0-2.94.707-3.857 1.818a.5.5 0 1 1-.771-.636A6.002 6.002 0 0 1 13.917 7H12.9A5.002 5.002 0 0 0 8 3zM3.1 9a5.002 5.002 0 0 0 8.757 2.182.5.5 0 1 1 .771.636A6.002 6.002 0 0 1 2.083 9H3.1z"/>
                              </svg>
                            <h5>Hybrid</h5>
                            </a>
                            
                        </div>
                    </div>
                </div>
                <div class="col-md-2"></div>
            </div>
        </div><br>
        <div class="col-md-12">
            <h3>Find Out Here</h3>
            <br>
            <div class="row g-3">
             <div class="col-md-2"></div>
             <div class="col-md-8"> 
                 <div class="col-md-6" style="text-align: right">
                    <input class="form-control" type="text" style="width:100%" placeholder="What are you looking for?" aria-label="default input example"> 
                 </div>
                 <div class="col-md-6" style="text-align: left"><button type="button" class="btn btn-info">Search</button></div>
                </div>
                <div class="col-md-2"></div>
            </div>
        </div>
         {{-- Web Browser --}}
        <div class="col-md-12" id="web-browser">
            <div class="row g-3" style="text-align: center">
                <div class="col-md-12" style="padding-top:10%;padding-bottom:10%">
                    <div class="row g-3">
                        <div class="col-md-12" >
                            <h3>Web Browser</h3>
                            <p>It made up from Laravel and Flutter that translating into JavaScript</p>
                        </div>
                        <div class="col-md-12">
                            <div class="row g-3">
                                @for ($i = 0; $i < 10; $i++)
                                    <div class="col-md-3">
                                        <div class="card">
                                            <div class="card-body">
                                                <h5 class="card-title">Card title</h5>
                                                <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
                                                <a href="#" class="btn btn-primary">Go somewhere</a>
                                            </div>
                                        </div>
                                    </div>
                                @endfor
                            </div>
                        </div>
                        <div class="col-md-12" style="text-align:center">
                            <a href="#"class="btn btn-outline-info">Explore More</a>
                        </div>
                    </div>
                </div>

            </div>
        </div>
         {{-- Mobile App --}}
         <div class="col-md-12" id="mobile-app">
            <div class="row g-3" style="text-align: center">
                <div class="col-md-12" style="padding-top:10%;padding-bottom:10%">
                    <div class="row g-3">
                        <div class="col-md-12" >
                            <h3>Mobile App</h3>
                            <p>Mobile App are Flagship Product at Software Development Area, because it's portable and easy-to-use. In this product will compatible in iOS, iPadOS, Android, and Android Tablet hybridly with single code base.</p>
                        </div>
                        <div class="col-md-12">
                            <div class="row g-3">
                                @for ($i = 0; $i < 10; $i++)
                                    <div class="col-md-3">
                                        <div class="card">
                                            <div class="card-body">
                                                <h5 class="card-title">Card title</h5>
                                                <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
                                                <a href="#" class="btn btn-primary">Go somewhere</a>
                                            </div>
                                        </div>
                                    </div>
                                @endfor
                            </div>
                        </div>
                        <div class="col-md-12" style="text-align:center">
                            <a href="#"class="btn btn-outline-info">Explore More</a>
                        </div>
                    </div>
                </div>

            </div>
        </div>
         {{-- Desktop App --}}
         <div class="col-md-12" id="desktop-app">
            <div class="row g-3" style="text-align: center">
                <div class="col-md-12" style="padding-top:10%;padding-bottom:10%">
                    <div class="row g-3">
                        <div class="col-md-12" >
                            <h3>Desktop App</h3>
                            <p>In desktop app, it will compatible with macOS, Windows, and Linux.</p>
                        </div>
                        <div class="col-md-12">
                            <div class="row g-3">
                                @for ($i = 0; $i < 10; $i++)
                                    <div class="col-md-3">
                                        <div class="card">
                                            <div class="card-body">
                                                <h5 class="card-title">Card title</h5>
                                                <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
                                                <a href="#" class="btn btn-primary">Go somewhere</a>
                                            </div>
                                        </div>
                                    </div>
                                @endfor
                            </div>
                        </div>
                        <div class="col-md-12" style="text-align:center">
                            <a href="#"class="btn btn-outline-info">Explore More</a>
                        </div>
                    </div>
                </div>

            </div>
        </div>
         {{-- Hybrid --}}
         <div class="col-md-12" id="hybrid">
            <div class="row g-3" style="text-align: center">
                <div class="col-md-12" style="padding-top:10%;padding-bottom:10%">
                    <div class="row g-3">
                        <div class="col-md-12" >
                            <h3>Hybrid</h3>
                            <p>Hybrid is a combination from all platform that they made up from single code base. And it's so responsive according screen dimension</p>
                        </div>
                        <div class="col-md-12">
                            <div class="row g-3">
                                @for ($i = 0; $i < 10; $i++)
                                    <div class="col-md-3">
                                        <div class="card">
                                            <div class="card-body">
                                                <h5 class="card-title">Card title</h5>
                                                <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
                                                <a href="#" class="btn btn-primary">Go somewhere</a>
                                            </div>
                                        </div>
                                    </div>
                                @endfor
                            </div>
                        </div>
                        <div class="col-md-12" style="text-align:center">
                            <a href="#"class="btn btn-outline-info">Explore More</a>
                        </div>
                    </div>
                </div>

            </div>
        </div>
    </div>
</div>

@stop
